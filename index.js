...
const cityList = fs_1.readFileSync('./world_map_medium.txt', 'utf8')
    .trim()
    .split('\n')
    .map(ramda_1.split(' '))
    .map(([name, ...roads]) => [name, {
...
const worldState = ramda_1.fromPairs(cityList);
 ...
 */
const step = () => {
    const aliensByCity = new Map();
    const citiesToDestroy = [];
    ...
    for (let i = alienList.length; i--;) {
        ...
        // calculate movement
        alienList[i].city = neighbouringCities[Math.floor(Math.random() * neighbouringCities.length)];
    }
};
...
for (let city in worldState) {
    console.log(city, ...ramda_1.toPairs(worldState[city].roads).map(ramda_1.join('=')));
}
